...
class Reader():
  def __init__(self, tfrecords_file, image_size=256,
    min_queue_examples=1000, batch_size=1, num_threads=8, name='',channels=3,for_type='train'):
    """
    Args:
  ...
  def feed(self,type='train'):
    ...
    with tf.name_scope(self.name):
      ...
      image = tf.image.decode_jpeg(image_buffer, channels=self.channels)
      image = self._preprocess(image)

      if self.type=='train':
        images,image_names = tf.train.shuffle_batch(
      ...
      if self.type=='test':
        images,image_names = tf.train.batch(
            [image, image_name], batch_size=self.batch_size, num_threads=1,
            capacity=self.min_queue_examples + 3*self.batch_size)
  ...
  def _preprocess(self, image):
    ...
    image.set_shape([self.image_size, self.image_size, self.channels])
    return image
