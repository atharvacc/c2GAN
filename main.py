...
def train():
  ...
  if FLAGS.type == 'train':
    ...
    if os.path.exists('fake') is False:
      os.makedirs('fake')
    if os.path.exists('fake/'+fake_folder+'/'+FLAGS.project+'fake_x') is False:
      os.makedirs('fake/'+fake_folder+'/'+FLAGS.project+'fake_x')
    if os.path.exists('fake/'+fake_folder+'/'+FLAGS.project+'fake_y') is False:
      os.makedirs('fake/'+fake_folder+'/'+FLAGS.project+'fake_y')
  
  if FLAGS.type == 'test':
    fake_folder=FLAGS.project+'_result'+datetime.now().strftime("%Y%m%d-%H%M")
    if os.path.exists('result') is False:
      os.makedirs('result')
    if os.path.exists('result/'+fake_folder+'/'+FLAGS.project+'result_x') is False:
      os.makedirs('result/'+fake_folder+'/'+FLAGS.project+'result_x')
    if os.path.exists('result/'+fake_folder+'/'+FLAGS.project+'result_y') is False:
      os.makedirs('result/'+fake_folder+'/'+FLAGS.project+'result_y')
  ...
  with graph.as_default():
    ...
    G_loss, D_Y_loss, F_loss, D_X_loss, fake_y, fake_x, fake_y_name, fake_x_name,x,y,test_loss = cycle_gan.model()
      ...
      while not coord.should_stop():
        ...
        if FLAGS.type == 'train':
          if step % 100 == 0:
            ...
            fake_y_path='fake/'+fake_folder+'/'+FLAGS.project+'fake_y/'+str(step)+str(fake_y_name_val)+'.png'
            misc.imsave(fake_y_path,fake_y_img)
            fake_x_img=np.squeeze(fake_x_val)
            fake_x_path='fake/'+fake_folder+'/'+FLAGS.project+'fake_x/'+str(step)+str(fake_y_name_val)+'.png'
            misc.imsave(fake_x_path,fake_x_img)

        if FLAGS.type == 'test':
          if step % 1 == 0:
            fake_y_img=np.squeeze(fake_y_val)
            fake_y_path='result/'+fake_folder+'/'+FLAGS.project+'result_y/'+str(fake_y_name_val)+'+'+str(step)+'.png'
            misc.imsave(fake_y_path,fake_y_img)
            fake_x_img=np.squeeze(fake_x_val)
            fake_x_path='result/'+fake_folder+'/'+FLAGS.project+'result_x/'+str(fake_x_name_val)+'+'+str(step)+'.png'
            misc.imsave(fake_x_path,fake_x_img)
          
        # train
        if FLAGS.type == 'train':
          _, G_loss_val, D_Y_loss_val, F_loss_val, D_X_loss_val, test_loss_val, summary = (
              sess.run(
                  [optimizers, G_loss, D_Y_loss, F_loss, D_X_loss,test_loss, summary_op],
